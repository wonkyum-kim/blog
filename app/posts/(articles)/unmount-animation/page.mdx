...
그리고 view-transition-name을 화면이 전환될 때만 부여한다면, 굳이 [같은 DOM이 아니라도 화면 전환 애니메이션이 적용된다.](https://developer.chrome.com/docs/web-platform/view-transitions/same-document?hl=ko#transitioning_elements_dont_need_to_be_the_same_dom_element)

예를 들어 같은 사진이지만, `/foo`에서는 100px x 100px이고, `/bar`에서는 200px x 200px인 경우,

/foo에서 /bar로 이동할 때, 두 사진의 view-transition-name을 동일하게 설정한다면 화면 전환 애니메이션이 적용될 수 있다.
...
- 일단 화면 전환 애니메이션이 시작하면, 시작한 애니메이션이 끝나기 전에 다른 화면 전환 애니메이션은 적용되지 않기 때문에 빠른 전환은 불가능.
- 빠른 전환이 필요하다면, framer-motion의 AnimatePresence를 사용해 볼 수 있지만, 버그가 있어서 이슈가 많이 올라왔음.
- 아직 크롬과 엣지에서만 적용할 수 있기 때문에 당분간은 framer-motion을 위주로 사용해야 할듯.
