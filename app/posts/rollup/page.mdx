...
번들링을 하는 과정에서 등장하는 것이 `tree-shaking`이다.

트리 쉐이킹은 살아있는 코드만을 포함시키는 것을 의미한다.

즉, 쓰이지 않는 코드를 제거하여 용량을 줄이는 역할을 한다.

그리고 함께 등장하는 것이 `minification`이다.
...
<CodeSelector codes={['package.json']} desc='package' />

```json {2, 4, 6, 13} showLineNumbers
{
  "type": "module",
  "scripts": {
...
- `"rm -rf dist/* && tsc && rollup -c"`를 통해, 이전 빌드 출력 결과물을 지우고, 번들링한다.
- `"exports"` 필드를 사용하여 실제 file system 상의 위치와 다른 모듈 지정자(module specifier)를 사용할 수 있게 된다.
- `"browserslist"`에 Babel에서 지원할 브라우저를 명시한다.

tsc는 각 파일에 해당하는 `.d.ts` 파일들을 `dist` 디렉터리에 생성하고, rollup은 `dist/esm`과 `dist/cjs`에 각각의 자바스크립트 파일을 생성한다.

모듈 지정자란, `import ... from ...`에서 from 뒤에 나오는 부분을 의미한다.

exports 필드를 사용하면 같은 모듈 지정자를 사용해도 모듈 시스템에 맞는 자바스크립트 파일을 불러올 수 있다.

<CodeSelector codes={['example.js']} desc='module-system-desc' />
...
<CodeSelector codes={['tsconfig.json']} desc='tsconfig' />

```json {3-6, 8} showLineNumbers
{
  "compilerOptions": {
    "module": "esnext",
...
- module은 `esnext`를 사용한다.
- moduleResolution은 `bundler`를 사용한다.
- tsc는 선언 파일만 출력하도록 한다.
- esModuleInterop는 ESM에서 CJS 모듈을 사용하는 라이브러리를 불러오는 문제를 해결해준다.

`module`은 컴파일되어 나온 자바스크립트가 사용할 모듈 시스템을 지정하는 옵션이다.

`moduleResolution`은 타입스크립트가 모듈 지정자를 디스크에 있는 파일에서 찾아내는 알고리즘을 지정하는 옵션이다.

moduleResolution을 bundler로 설정하는 이유는, tsc가 아니라 번들러가 파일들을 찾기 때문이다.

bundler로 설정했을 때의 추가적으로 얻는 장점은 `느슨한 import`를 사용할 수 있다는 것이다.

<CodeSelector codes={['exmaple.ts']} desc='bundler' />

```ts
// 상대 경로에 있는 파일들을 import 할 때, 파일의 확장자를 적지 않아도 된다.
import { foo } from './foo'
```
...
따라서 `esModuleInterop`을 true로 설정해서 생길수도 있는 문제를 피할 수 있다.
...
<CodeSelector codes={['shell']} desc='babel/preset-env' />
...
<CodeSelector codes={['babel.config.json']} desc='babel.config' />
...
<CodeSelector codes={['shell']} desc='install rollup' />
...
<CodeSelector codes={['rollup.config.js(기본)', 'rollup.config.js(상세)']} desc='rollupconfigjs' />
...
ESM과 CJS 코드 모두 빌드하는 것이 목표이므로 상세 코드를 기반으로 작성해본다.
...
<CodeSelector codes={['shell']} desc='plugintypescript' />
...
<CodeSelector codes={['shell']} desc='pluginbabel' />
...
<CodeSelector codes={['shell']} desc='pluginnoderesolve' />
...
<CodeSelector codes={['shell']} desc='plugincommonjs' />
...
<CodeSelector codes={['shell']} desc='pluginterser' />
...
번들링을 하면서 가장 헷갈렸던 부분은 CJS와 ESM에 따라서 타입스크립트 선언 파일도 달라지는가? 였다.
