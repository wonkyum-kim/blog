...
<TOC headings={getHeadingsBySlug('rollup')} />
...
<ToolTip
  ...
  codeIndex='0'
  content={{
    '2': '현재 프로젝트의 모듈 시스템은 ESM',
    '4': '이전 빌드 출력 결과물을 지우고 컴파일, 번들링한다.',
    '6': '실제 file system 상의 위치와 다른 모듈 지정자(module specifier)를 사용',
    '13': 'Babel에서 지원할 브라우저를 명시',
  }}
/>
...
<ToolTip
  ...
  codeIndex='0'
  content={{
    '3': '컴파일되어 나온 자바스크립트가 사용할 모듈 시스템을 지정하는 옵션',
    '4': '타입스크립트가 모듈 지정자를 디스크에 있는 파일에서 찾아내는 알고리즘을 지정',
    '6': 'tsc는 선언 파일만 출력하도록 한다.',
    '8': 'ESM에서 CJS 모듈을 사용하는 라이브러리를 불러오는 문제를 해결',
  }}
/>

`moduleResolution`을 bundler로 설정하는 이유는, tsc가 아니라 번들러가 파일들을 찾기 때문이다.
...
<CodeSelector names={['rollup.config.js']} desc='rollupconfigjs' />

```js {17, 20-21, 28-29} showLineNumbers
...
<ToolTip
  ...
  content={{
    '17': '번들링할 코드의 진입점',
    '20': '번들링 된 파일의 이름',
    '21': '번들링된 파일이 사용할 모듈 시스템',
    '28': '현재 사용하고 있는 외부 모듈(dependencies).\n  여기에 쓰여진 외부 모듈은 번들링된 코드에 직접 포함되지 않고 import 된다',
    '29': '사용할 플러그인',
  }}
/>
