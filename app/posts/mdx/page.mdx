...
`layout.tsx`에서 현재 url을 확인하고, 각 컴포넌트에 slug를 건네주면, 컴포넌트는 slug에 해당하는 frontmatter를 찾을 수 있다.

하지만 layout.tsx를 서버 컴포넌트로 작성할 것이기 때문에 현재 url을 확인하는 `usePathname` 훅을 사용할 수 없다.

서버 컴포넌트에서 현재 url을 확인하려면 `middleware.ts`를 수정해야 한다.

HTTP 응답 헤더에 현재 url을 넣어주는 방법이다.

<CodeSelector names={['middleware.ts', 'app/posts/layout.tsx']} desc='middleware' />
