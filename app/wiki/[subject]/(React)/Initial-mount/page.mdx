...
- `stateNode`: 일반적으로 fiberNode와 연관된 local state를 가리키는데 사용되며, 주로 DOM 노드를 가리키는데 사용된다.
...
렌더링을 스케줄 하는 과정은 대략적으로 아래와 같이 진행된다.
...
매번 새로운 렌더링이 시작할 때마다, current Fiber 트리의 HostRoot로부터 새로운 workInProgress가 생성된다.

이 FiberNode를 `전역 변수 workInProgress`로 설정해둠으로써 어디서든 접근할 수 있게 하고 있다.

### performUnitOfWork()

`workLoopSync()`는 그냥 while 루프이며, workInProgress가 null이 될 때까지 실행된다.

workInProgress는 dfs와 같은 방식으로 이동하고 `performUnitOfWork()`를 실행한다.

즉, 자식 방향으로 계속 이동하다가 더 이상 자식이 없으면 형제 노드로 이동 한다는 의미다.

workInProgress는 현재 HostRoot이며, null이 된다는 의미는 HostRoot로부터 WorkInProgress Fiber 트리를 전부 완성했다는 의미가 된다.
...
performUnitOfWork()에서 각 노드마다 `beginWork()`가 호출된다.
...
performUnitOfWork()는 `beginWork()`를 호출하여 FiberNode의 실제 렌더링을 발생시킨다. 이때 workInProgress.tag에 따라서 다른 렌더링을 진행하게 된다.

<CodeSelector names={['beginWork']} desc='beginWork' />

```ts showLineNumbers
if (current !== null) {
  // memoizedProps와 pendingProps는
