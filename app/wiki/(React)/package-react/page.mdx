...
- Internal: React 내부에서 사용되는 객체(지만 외부로 노출되어 있다.)
- Legacy API: 현재는 사용이 권장되지 않는 API(클래스형 컴포넌트 등...)
- Component: 미리 정의된 React 컴포넌트 (react는 단순히 export 할 뿐이고, 정의는 shared에 구현은 다른 곳에 있는 것 같다.)
- API: 컴포넌트를 정의하는데 도움이 되는 API
- Hook: React 훅 (react가 아니라 react-reconciler에 구현되어 있는 것 같아서 넘어간다.)
- Unstable: 여기서는 다루지 않고 넘어간다.
...
실제 React 내부 코드에서는 저 긴 이름 대신에 `ReactSharedInternals`로 사용한다.

외부에서 사용은 권장하지 않지만 사용할 수 있도록은 해두었다.
...
`ReactElement` 함수를 사용하여 새로운 React 엘리먼트를 반환한다.

그렇다면 React 엘리먼트란 무엇인가?

ReactElement의 구현을 살펴보면 아래와 같이 type, key, ref, props를 가지고 있는 객체를 의미한다.
...
이 객체가 DOM으로 변환되는 과정은 나중에 다른 패키지와 같이 살펴보기로 한다.
...
[React 엘리먼트](#createelement)인지 확인해주는 함수다. 그래서 구현은 매우 간단하다.
...
구현체: [src/ReactHooks.js](https://github.com/facebook/react/blob/main/packages/react/src/ReactHooks.js)
...
<CodeSelector names={['src/ReactHooks.js']} desc='ex4' />
...
`ReactSharedInternals`는 아까 보았던 `__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE`와 같고, `H` 프로퍼티는 Dispatcher를 나타낸다.

use 뿐만 아니라 모든 훅들도 Dispatcher에 상세 구현이 포함되어 있으므로 react 패키지에서는 역할에 대해서만 살펴보고 넘어간다.
...
즉, 언젠가 지원해줄 것이라고 기대해 볼 수 있다.

다시 서버 컴포넌트로 돌아와서, cache는 마치 `ReactQuery`와 비슷하게 작동한다.
...
`src/ReactCacheImpl.js`에서 dispatcher는 `ReactSharedInternals.A`인데, [H](#use)와는 달리 AsyncDispatcher다.

> TODO: AsyncDispatcher에 대해 알아보기

그리고 cache 함수의 구현을 보면 인자의 개수만큼 `cacheNode`를 계속 갱신하는데 아마도 이런 방식의 구현은 나중에 바뀌지 않을까 생각해본다.

또한 아직까지 ReactQuery나 Next.js fetch에 비해 기능이 많이 부족하기 때문에 대체하기는 힘들어보인다.
...
구현을 살펴보면 `ReactSharedInternals`의 T와 S를 사용한다.

아직까지 이 부분의 동작이 정확히 어떻게 되는지는 알지 못하지만, T를 prevTransition으로 따로 복사하여 S를 통해 논블로킹 호출에 전달하고 다시 T에 prev를 복원시키는 것으로 보인다.
