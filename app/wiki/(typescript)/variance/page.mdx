...
좀 더 큰 집합을 슈퍼타입, 작은 집합을 서브타입이라고 하자.

<CodeSelector names={['ts']} desc='type' />
...
별다른 설명 없이도, A가 B보다 더 큰 집합인 것을 알 수 있다.

## variance

타입스크립트에서 variance는 제네릭 타입 `F<T>`가 타입 파라미터 T에 대해서 어떻게 달라지는지 설명한다.
...
`반변적`이란 `T extends U` 이면, `F<U> extends F<T>`이 되는 것을 의미한다.
